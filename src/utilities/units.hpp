...
struct units {
    ...
    static constexpr double me_c2 = me * clight * clight;       // electron rest energy [erg]

    static constexpr double k_boltz = 1.380649e-16;             // Boltzmann constant [erg/K]

    static constexpr double sigma_sb = 5.670374419e-5;          // Stefan-Boltzmann [erg cm^-2 s^-1 K^-4]
    static constexpr double arad = 4.0 * sigma_sb / clight;     // radiation constant [erg cm^-3 K^-4]

    static constexpr double sigma_thomson = 6.652458732160e-25;  // Thomson cross section [cm^2]
    ...
    static constexpr double planck_constant = 6.62607015e-27;   // Planck constant [erg s]

    static constexpr double r_e2 = sigma_thomson / (8.0 * std::numbers::pi / 3.0); // classical electron radius squared [cm^2]
