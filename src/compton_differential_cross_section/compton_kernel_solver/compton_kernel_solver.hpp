 ...
 *   2.  Double power series (speculative) -- always attempted; accepted
 *       when self-error < quadrature_self_tol and (for sigma_E) non-negative.
 *   3.  Gauss-Laguerre Q64 -- tried only when tau_alpha_max is below
 *       quadrature_useful_threshold (otherwise Q64 cannot converge).
 *   4.  DD power series + conditional Asymp-DD cross-validation --
 *       fallback when both double PS and Q64 fail.  Asymp-DD is skipped
 *       when PS-DD reports clearly trustworthy results.
 *
...
/// Skip Q64 when tau_alpha_max exceeds this value (Q64 cannot converge
/// in this regime at low gamma).
constexpr double QUADRATURE_USEFUL_THRESHOLD = 0.15;
} // namespace constants
...
public:
     ...
     * @param quadrature_self_tol  Accept double PS or quadrature result when
     *        its self-reported relative error is below this tolerance.  Also
     *        controls the acceptance threshold for the speculative double PS
     *        attempt.
     ...
     * @param quadrature_useful_threshold  Skip Q64 when tau_alpha_max
     *        exceeds this value (Q64 cannot converge in this regime).
     */
    ...
    ComptonKernelSolver(
        ...
        double quadrature_useful_threshold = constants::QUADRATURE_USEFUL_THRESHOLD);
...
private:
    ...
    double quadrature_useful_threshold_;
